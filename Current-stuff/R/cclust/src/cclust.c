...
/*****************************************************************/
/*
 *  Copyright (C)1998 Evgenia Dimitriadou
 *               1998 Friedrich Leisch and Andreas Weingessel
 *
	   ...
	   double *par,int *t, int *verbose)
{
  ...
  for (n=0;n<*xcols;n++){
    centers[cluster[k]+(*ncenters)*n]+=(1.0/pow(t[cluster[k]],par1))*(x[k+(*xrows)*n]-centers[cluster[k]+(*ncenters)*n]);
  }
    }
    /*expon. decaying l.r*/
    else if(*methrate==1){
  ...
  for (n=0;n<*xcols;n++){
  centers[cluster[k]+(*ncenters)*n]+=e*(x[k+(*xrows)*n]-centers[cluster[k]+(*ncenters)*n]);
    }
   }           
  }
  /*ERROR MINIMIZATION*/
